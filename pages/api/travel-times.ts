...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // DM limits: for 1 origin, <=25 destinations per request is safe; also keep element count reasonable
    const CHUNK = 25;
    const destChunks: Dest[][] = [];
    for (let i=0;i<destinations.length;i+=CHUNK) destChunks.push(destinations.slice(i,i+CHUNK));

    async function matrix(mode: "driving"|"walking", dests: Dest[]): Promise<number[]> {
      ...
      if (data.status !== "OK") throw new Error(`DistanceMatrix API non-OK ${mode.toUpperCase()} ${data.status}`);
      const row = data.rows?.[0]?.elements ?? [];
      return row.map((e:any) => (e.status === "OK" ? e.duration.value : null));
    }

    const allDriving: number[] = []; const allWalking: number[] = [];
    for (const chunk of destChunks) {
      const [d,w] = await Promise.all([matrix("driving", chunk), matrix("walking", chunk)]);
      allDriving.push(...d); allWalking.push(...w);
    }

    // Stitch back to ids order
    ...
    return res.status(500).json({ error: e?.message || "Failed to fetch travel times" });
  }
}
